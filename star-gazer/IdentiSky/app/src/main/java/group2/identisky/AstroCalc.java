...
public class AstroCalc {
    ...
    public static void main(String[] args){
        ...
        SkyObject obj = new SkyObject("obj", 79.17,46.0);
    ...
    public static boolean isVisible(SkyObject obj, double horizon, double lowerAz, double upperAz) {
        double alt = obj.getAltitude();
        double az = obj.getAzimuth();
        if (alt > horizon) {
            if (lowerAz > upperAz) {
                if ((lowerAz <= az && az <= 360) || (0 <= az && az <= upperAz)){
                    return true;
                }
            }
            else if (obj.getAzimuth() >= lowerAz && obj.getAzimuth() <= upperAz){
                return true;
            }
        }
        return false;
